package uxokpro1234.optimizer.utils;
...
import uxokpro1234.optimizer.CrystalOptimizer;
...
public class PacketUtils {
    ...
    public static boolean canPlaceCrystalServer(BlockPos block) {
        BlockState blockState = CrystalOptimizer.mc.world.getBlockState(block);
        if (!blockState.isOf(Blocks.OBSIDIAN) && !blockState.isOf(Blocks.BEDROCK))
            return false;
        BlockPos blockPos2 = block.up();
        if (!CrystalOptimizer.mc.world.isAir(blockPos2))
            return false;
        double d = blockPos2.getX();
        double e = blockPos2.getY();
        double f = blockPos2.getZ();
        List<Entity> list = CrystalOptimizer.mc.world.getOtherEntities(null, new Box(d, e, f, d + 1.0D, e + 2.0D, f + 1.0D));
        return list.isEmpty();
    }

    public static BlockState getBlockState(BlockPos pos) {
        return CrystalOptimizer.mc.world.getBlockState(pos);
    }
    ...
    public static ActionResult interact(Vec3d vec3d, Direction dir) {
        ...
        return CrystalOptimizer.mc.interactionManager.interactBlock(CrystalOptimizer.mc.player, CrystalOptimizer.mc.player.getActiveHand(),result);
    }
    ...
    public static Vec3d getClientLookVec() {
        return getPlayerLookVec(CrystalOptimizer.mc.player);
    }


    public static int getPing() {
        if (CrystalOptimizer.mc.getNetworkHandler() == null) return 0;

        PlayerListEntry playerListEntry = CrystalOptimizer.mc.getNetworkHandler().getPlayerListEntry(CrystalOptimizer.mc.player.getUuid());
